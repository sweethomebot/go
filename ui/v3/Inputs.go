...
func NewFormInput(ch core.Channel, transModul string, input FormInput, value string) interface{} {
	...
	case "select":
		...
		return choice
	case "callback":
		fncName := FunctionCallApi
		if input.Button.SendForm {
			fncName = FunctionSendForm
		}
		fnc := NewFunction(fncName, core.JsonEncode(map[string]string{"Url": input.Button.Url}) )
		btn := NewButtonColor(core.TransPlugin(ch, transModul, input.Button.Title), fnc , input.Button.Color)
...
func NewInputCallback(title string, name string, button Button ) InputCallback {
	return InputCallback{Type: "input_callback", Disabled: false, Title: title, Name: name, Button: button}
}